...
Fill in the blank to inspect the first 6 rows of the new data.frame. 
...
* `r fa(name = "question-circle")` Not all countries are included in the gapminder dataset. Can you find your country there?
...
3. **Countries missing from data.frame** `r fa(name = "flag")` There are several countries in `sfafricountries` that are not present in `africa_gap`.
...
| Congo, Rep.   | Republic of Congo     | **Congo**      |
  ...
  - In the table above, we've noted the changes we need to make in order to match the country names in the `africa_gap` and `sfafricountries` data sets and to bring them in line with the Inter-institutional Country Style Guide. 
...
* Depending on the names you wish to have, you can change either data set or both!
...
Fill in the blank to recode the country *Republic of Congo* in `sfafricountries` to *Congo*.


```{r recode_sfafricountries_ex, exercise = TRUE}
...
```{r recode_sfafricountries, echo = FALSE}
...
```{r africa_gap_anti_join2, exercise = TRUE, exercise.setup = "recode_sfafricountries"}
...
```{r spatial_anti_join_ex2, exercise = TRUE, exercise.setup = "recode_sfafricountries"}
...
Now that we've done the prep work so that the countries match, we can now join the two data sets together.
...
- Note that the join takes the same form as the `anti_join`. Because we are joining the `africa_gap` data to the `sfafricountries` data set. 



```{r final_left_join2, echo = FALSE}

africa_df <- dplyr::left_join(x = sfafricountries, 
                                        y = africa_gap, 
                                        by = c("name_long" = "country")
                                        )
...
```{r final_left_join, exercise = TRUE, exercise.eval = TRUE, exercise.setup = "recode_sfafricountries"}
...
Let's make a couple of changes to the data set before plotting.
...
```{r plot1, exercise = TRUE, exercise.setup = "final_left_join2"}
